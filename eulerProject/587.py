...
# def calculate_y(n):
#     return Decimal(n+1-sqrt(2*n))/Decimal(n**2+1)

# def triangle_area(n):
#     y = calculate_y(n)
#     x = n*y
#     return Decimal(x*y/2)

# def concave_triangle_area(n):
#     x = n*calculate_y(n)
#     x = 1 - x
#     integral = Decimal(asin(x)/2)+ Decimal(x/2)*Decimal(sqrt(1-x**2))
#     return Decimal(x) - Decimal(integral)

# def total_area(n):
#     return Decimal(triangle_area(n)) + Decimal(concave_triangle_area(n))

# L_section = Decimal(1*1) - Decimal(pi*1**2/4)

# n = 1
# while (total_area(n)/L_section) > 0.001:
#     n += 1
# print (n)